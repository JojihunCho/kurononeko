...
#define ASSERT_(x, n, line, sub) if ((x) != n) { printf(1, "wrong in line %d %d %d\n", line, x, n); exit(); }
...
void test_pwrite2() {
  ...
  readtest("testfile", "qzxrt", __LINE__);
...
void test_pwrite3() {
  ...
  int fd = open("testfile", O_CREATE|O_WRONLY);
  ASSERT(pwrite(fd, (char*)buffer, sizeof(buffer), 0), sizeof(buffer));
  close(fd);
